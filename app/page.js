...
const blank = {
  ...
  oil_spec:'0W-20 Dexos1 Gen3',
  oil_brand:'Mobil 1',
  filter_brand:'ACDelco',
  cost:'',
  receipt_photo_url:'',
  odometer_photo_url:'',
  notes:''
...
export default function Page(){
  ...
  const [status,setStatus]=useState(supabaseReady?'Loading cloud data...':'Missing Supabase environment variables');

  useEffect(()=>{ if(supabaseReady) loadAll(); },[]);

  async function loadAll(){
    setStatus('Loading cloud data...');
    const { data: settings } = await supabase.from('settings').select('*').eq('user_id', USER_ID).single();
    if(settings){
      setCurrentMileage(settings.current_mileage || '');
      setIntervalMiles(settings.interval_miles || 5000);
    }
    const { data, error } = await supabase.from('maintenance_records').select('*').eq('user_id', USER_ID).order('mileage',{ascending:false});
    if(error){ setStatus('Cloud load error: '+error.message); return; }
    setEntries(data || []);
    setStatus('Cloud synced');
  }

  async function saveSettings(nextCurrent=currentMileage,nextInterval=intervalMiles){
    if(!supabaseReady) return;
    await supabase.from('settings').upsert({
      user_id: USER_ID,
      current_mileage: Number(nextCurrent || 0),
      interval_miles: Number(nextInterval || 5000),
  ...
  async function uploadPhoto(file, type){
    if(!file) return '';
    if(!supabaseReady){ alert('Supabase is not configured.'); return ''; }
    const safeName = file.name.replace(/[^a-zA-Z0-9._-]/g,'-');
    const path = `${USER_ID}/${Date.now()}-${type}-${safeName}`;
  ...
  async function handleIntervalChange(value){
    setIntervalMiles(value);
  }

  async function saveDashboardSettings(){
    setStatus('Saving settings...');
    await saveSettings(currentMileage, intervalMiles);
  }
  ...
  if(!supabaseReady){
    return <main style={s.page}><section style={s.hero}><h1>Missing Supabase Setup</h1><p>Add NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY in Vercel Environment Variables, then redeploy.</p></section></main>
  }

  return <main style={s.page}>
    <section style={s.hero}>
      <h1 style={s.h1}>Malibu Maintenance Tracker</h1>
      <p style={s.muted}>Cloud sync with Supabase • records and photos work across browsers/devices</p>
    ...
    <section style={s.cards}>
      ...
      <Card t="Total Cost" v={`$${total.toFixed(2)}`} sub={`${entries.length} cloud records`} />
    </section>

    <section style={s.grid}>
      <div style={s.panel}>
        <h2>Dashboard Settings</h2>
        <Field label="Current Mileage"><input style={s.input} type="number" value={currentMileage} onChange={e=>handleCurrentChange(e.target.value)} /></Field>
        <Field label="Oil Interval Miles"><input style={s.input} type="number" value={intervalMiles} onChange={e=>handleIntervalChange(e.target.value)} /></Field>
        <button style={s.primary} onClick={saveDashboardSettings}>Save Dashboard Settings</button>
      </div>
      ...
      <div style={s.panel}>
        ...
        <div style={s.form}>
          <Photo label="Receipt Photo" value={form.receipt_photo_url} onUploaded={url=>update('receipt_photo_url',url)} />
          <Photo label="Odometer Photo" value={form.odometer_photo_url} onUploaded={url=>update('odometer_photo_url',url)} />
        </div>

        <button style={s.primary} onClick={addEntry}>Save Record to Cloud</button>
      </div>
    </section>

    <section style={s.panel}>
      <h2>Maintenance Records</h2>
      {entries.length===0 ? <p style={s.empty}>No records yet.</p> :
      <div style={{overflowX:'auto'}}><table style={s.table}>
        <thead><tr>{['Date','Mileage','Service','Oil','Filter','Cost','Photos','Notes',''].map(x=><th style={s.th} key={x}>{x}</th>)}</tr></thead>
        <tbody>{entries.map(e=><tr key={e.id}>
          <td style={s.td}>{e.date}</td>
          <td style={s.td}>{Number(e.mileage).toLocaleString()}</td>
          <td style={s.td}>{e.service_type}</td>
          <td style={s.td}>{e.oil_brand} • {e.oil_spec}</td>
          <td style={s.td}>{e.filter_brand}</td>
          <td style={s.td}>{e.cost?`$${Number(e.cost).toFixed(2)}`:'—'}</td>
          <td style={s.td}><div style={{display:'flex',gap:8}}>
            {e.receipt_photo_url && <a href={e.receipt_photo_url} target="_blank"><img src={e.receipt_photo_url} style={s.thumb}/></a>}
            {e.odometer_photo_url && <a href={e.odometer_photo_url} target="_blank"><img src={e.odometer_photo_url} style={s.thumb}/></a>}
          </div></td>
          <td style={s.td}>{e.notes||'—'}</td>
          <td style={s.td}><button style={s.del} onClick={()=>deleteEntry(e.id)}>Delete</button></td>
        </tr>)}</tbody>
      </table></div>}
    </section>
  </main>

  function Photo({label,value,onUploaded}){
    const [uploading,setUploading]=useState(false);
    return <div style={s.upload}><b>{label}</b><input style={{marginTop:10}} type="file" accept="image/*" onChange={async e=>{
      const file=e.target.files?.[0]; if(!file)return;
      setUploading(true);
      const url=await uploadPhoto(file,label.toLowerCase().replaceAll(' ','-'));
      if(url) onUploaded(url);
      setUploading(false);
    }}/>{uploading&&<p>Uploading...</p>}{value&&<img src={value} style={s.preview}/>}</div>
  }
}
...
const s={
 ...
 label:{display:'grid',gap:7,fontWeight:700,color:'#334155',marginBottom:14}, input:{border:'1px solid #cbd5e1',borderRadius:14,padding:12,fontSize:15}, primary:{width:'100%',background:'#020617',color:'white',border:0,borderRadius:16,padding:16,fontSize:18,fontWeight:800,cursor:'pointer'},
